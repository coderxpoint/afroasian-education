...
const EnrollNowFormSection: React.FC = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        const response = await axios.get<Contact[]>("/api/enrollnow"); // Specify the expected response type
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await axios.delete(`/api/enrollnow?id=${id}`);
  ...
  return (
    <div className="w-full  flex px-4">
      <div className="container mx-auto justify-center">
        <h2 className="text-xl font-semibold mb-2">Enroll Now Form</h2>
        {error && <p className="text-red-500">{error}</p>}
        <Table>
          <TableCaption>A list of submitted Enroll Now.</TableCaption>
          <TableHeader>
            <TableRow>
...
export default EnrollNowFormSection;
