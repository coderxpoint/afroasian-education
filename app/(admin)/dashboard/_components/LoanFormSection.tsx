...
const LoanFormSection: React.FC = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        const response = await axios.get<Contact[]>("/api/loan-form"); // Specify the expected response type
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await axios.delete(`/api/loan-form?id=${id}`);
          ...
          <TableBody>
            {contacts.map((contact) => (
              <TableRow key={contact.id}>
                ...
                <TableCell>{contact.bank}</TableCell>
