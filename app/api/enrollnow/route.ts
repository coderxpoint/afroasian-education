...
export const GET = async (request: Request) => {
  ...
  const query = "SELECT * FROM enroll_now_submissions";
...
export async function POST(request: Request) {
  ...
  try {
    const connection = await createConnection();
    const query =
      "INSERT INTO enroll_now_submissions (name, email, phone, university, program_name) VALUES (?, ?, ?, ?, ?)";
...
export async function DELETE(request: Request) {
  ...
  try {
    const connection = await createConnection();
    const query = "DELETE FROM enroll_now_submissions WHERE id = ?";
