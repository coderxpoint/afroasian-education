...
export function ApplyUniversity({ defaultUniversity }: ApplyUniversityProps) {
  ...
  //   formData.append("subject", "Applied To University");
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const backendResponse = await fetch("/api/university", {
        method: "POST",
        headers: {
      ...
      <DialogContent className="sm:max-w-[425px] bg-gradient-to-br from-[#f9fafb] to-[#f3f4f6] rounded-lg border border-gray-200 shadow-2xl">
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <Select
              onValueChange={(value) => form.setValue("program_name", value)}
            >
              <SelectTrigger className="rounded-lg border-gray-300 focus:border-[#e86034] focus:ring-[#e86034]">
